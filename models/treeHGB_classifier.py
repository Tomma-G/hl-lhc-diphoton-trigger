#!/usr/bin/env python3
"""
Histogram-based gradient-boosted tree classifier for photon-versus-jet
discrimination using track-based features engineered from per-event csv files.
...
- saves scores, ROC curves, plots, and timing summaries
...
DEFAULT_OUT_DIR = PROJECT_ROOT / "final_outputs" / "treehgb_classifier"
...
def plot_fake_vs_acceptance(roc_df, out_path: Path, auc_val, model_label="Tree HGB classifier"):
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Run the histogram-based gradient-boosted tree photon-versus-jet classifier."
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
