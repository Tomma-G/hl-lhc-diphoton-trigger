...
def main():
    ...
    # strongest fair starting point from your earlier tree tests:
    # keep object kinematics, drop the 4 ratio features
    ...
    importances.to_csv(os.path.join(out_dir, "feature_importances_gain.csv"), index=False)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "feature_importances_top15.png"), dpi=200)
    plt.close()

    perm_df, base_auc_perm = permutation_importance_auc(model, X_val, y_val, feature_names)
    perm_df.to_csv(os.path.join(out_dir, "feature_importances_permutation_auc.csv"), index=False)
    ...
    plt.ylabel("Feature")
    plt.title("Top 15 Permutation Importances (AUC drop)")
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "feature_importances_permutation_top15.png"), dpi=200)
    plt.close()
