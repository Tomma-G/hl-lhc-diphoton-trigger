...
Main outputs:
  ...
  - track_ptmin_reduction.csv (diagnostic: effect of trk_pt_min on track multiplicity)

Scan support:
  --fast-scan  : skips plots + mgg and other heavy outputs, but still writes:
                 iso_values.csv, acceptance_vs_fake_rate_points.csv, roc.csv,
                 ntracks*.csv, track_reduction.csv, track_ptmin_reduction.csv
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "scan mode: skips plots + mgg and other heavy outputs, but still writes "
            "iso_values.csv + acceptance_vs_fake_rate_points.csv + roc.csv + ntracks*.csv + "
    ...
    info(f"track preselection (USED FOR FEATURES): tracks within dR(track, any photon) < {args.dr_track_keep}")
    info("features: photons use photon-conditioned tracks; jets use full track collection")
    info(f"track pT cut: pT > {args.trk_pt_min:g} GeV (applies to features + diagnostics)")

    if args.fast_scan:
        info(
            "fast-scan: ON (skipping plots + mgg; still writing iso_values.csv + ROC points + roc.csv + "
            "ntracks*.csv + track diagnostics CSVs)"
        )
    ...
    # diagnostic: effect of trk_pt_min cut on track multiplicity
    ...
    for ev in chosen:
        ...
        # feature inputs:
        # - photons use photon-conditioned tracks (df_trk_phclose)
        # - jets use full track collection (df_trk_all)
        trk_ph_eta = df_trk_phclose["eta"].to_numpy(dtype=float) if len(df_trk_phclose) else np.zeros(0, dtype=float)
        trk_ph_phi = df_trk_phclose["phi"].to_numpy(dtype=float) if len(df_trk_phclose) else np.zeros(0, dtype=float)
        trk_ph_pt  = df_trk_phclose["pT"].to_numpy(dtype=float)  if len(df_trk_phclose) else np.zeros(0, dtype=float)
        ...
        # # ---- photons: features use photon-conditioned tracks (trk_ph_*) ----
        for i, pho in df_ph.iterrows():
            pho_eta = float(pho["eta"])
            pho_phi = float(pho["phi"])
            pho_pt = float(pho["pT"])

            ntrk = count_tracks_near_object_fast(pho_eta, pho_phi, trk_ph_eta, trk_ph_phi, args.dr_track)
            iso = track_iso_scalar_sum_pt_fast(
                pho_eta, pho_phi, pho_pt, trk_ph_eta, trk_ph_phi, trk_ph_pt, args.iso_dr
            )

            ntrk_iso = count_tracks_near_object_fast(pho_eta, pho_phi, trk_ph_eta, trk_ph_phi, args.iso_dr)
            photon_total_ntracks.append(int(len(trk_ph_pt)))
        ...
        # # ---- jets: features use ALL tracks (trk_all_*) ----
    ...
    # ---- track pT-min diagnostic (this is what trk_pt_min actually changes) ----
    pt_red_path = os.path.join(out_dir, "track_ptmin_reduction.csv")
    pt_frac = (
        float(n_trk_after_pt_total) / float(n_trk_before_pt_total)
        if n_trk_before_pt_total > 0
        else float("nan")
    )
    ...
    ).to_csv(pt_red_path, index=False)
    info(f"wrote {pt_red_path}")
