...
def roc_from_iso(ph_iso: np.ndarray, jet_iso: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # collapse duplicates in FPR by taking the best (max) TPR at that FPR
    ...
    # enforce ROC envelope: TPR should not decrease as FPR increases
...
def main() -> None:
    ...
    out_dir = (
        ...
        else os.path.abspath("C:/Users/Tom Greenwood/Desktop/University/Year 4/SEM 2/SH Project/week5/analysis_v8_outputs")
    )
    os.makedirs(out_dir, exist_ok=True)
    ...
    for ev in chosen:
        ...
        # feature inputs (USED FOR FEATURES):
        # - photons use full track collection (trk_all_*)
        # - jets use full track collection (trk_all_*)
        ...
        # keep photon-close tracks only for diagnostics (do not use for features)
        ...
        # ---- photons: features use ALL tracks (trk_all_*) ----
    ...
    if np.any(np.isfinite(ph_iso)):
        ...
        plt.figure()
        plt.hist(vals, bins=200, range=(0.0, 0.02), log=True)
