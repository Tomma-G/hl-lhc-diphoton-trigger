...
Main outputs:
  ...
  - track_reduction.csv (global track preselection efficiency)
...
def _dr_mask_to_object(trk_eta: np.ndarray, trk_phi: np.ndarray, obj_eta: float, obj_phi: float, dr_max: float) -> np.ndarray:
    if trk_eta.size == 0:
...
def read_csv_maybe_gzip(path: str, **kwargs) -> pd.DataFrame:
    """Read CSV with optional gzip compression."""
...
def prepare_data_dir(path: str) -> str:
    ...
    # cache extraction next to the tarball
    ...
    with tarfile.open(abs_path, "r:gz") as tar:
        # Python 3.12+ supports filter= (avoids the 3.14 behaviour-change warning)
        try:
...
def main() -> None:
    ...
    # (3) quiet mode for subprocess scans
    parser.add_argument(
        "--quiet",
        action="store_true",
        default=False,
    ...
    out_dir = (
        ...
        else os.path.abspath(os.path.join(args.data_dir, "..", "analysis_outputs_final"))
    )
    ...
    for ev in chosen:
        ...
        # jet-photon overlap removal (still needed in fast-scan: iso for jets uses kept jets)
        kept = []
        ...
        for i, pho in df_ph.iterrows():
            ...
            # vectorised track count + iso
            ntrk = count_tracks_near_object_fast(pho_eta, pho_phi, trk_ph_eta, trk_ph_phi, args.dr_track)
            iso = track_iso_scalar_sum_pt_fast(pho_eta, pho_phi, pho_pt, trk_ph_eta, trk_ph_phi, trk_ph_pt, args.iso_dr)
    ...
    # summary + write track_reduction.csv (ONCE)
    ...
    # Fast-scan: stop here (this is the point)
    if args.fast_scan:
        return

    # everything below is full-analysis mode
    ...
    # fixed photon-efficiency operating point
    TARGET_TPR = 0.99
