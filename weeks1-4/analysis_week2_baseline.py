...
def roc_from_iso(ph_iso: np.ndarray, jet_iso: np.ndarray) -> Tuple[np.ndarray, np.ndarray, float, float]:
    ...
    cuts = np.unique(np.concatenate([ph, jt]))
    cuts = np.sort(cuts)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Isolation cone ΔR for LHC-style track isolation",
    )
    parser.add_argument("--iso-out-csv", default="iso_values.csv", help="Output CSV with per-object isolation values")
    parser.add_argument("--iso-photons-png", default="iso_photons.png", help="Histogram of isolation for photons")
    parser.add_argument("--iso-jets-png", default="iso_jets.png", help="Histogram of isolation for jets")
    parser.add_argument("--roc-png", default="acceptance_vs_fake_rate.png", help="Acceptance vs fake rate (ROC) plot")
    ...
    for ev in chosen:
        ...
        for i, pho in df_ph.iterrows():
            ...
            iso = track_iso_scalar_sum_pt(pho_eta, pho_phi, pho_pt, df_trk, args.iso_dr)
        ...
        for i, jet in enumerate(kept):
            ...
            iso = track_iso_scalar_sum_pt(jet_eta, jet_phi, jet_pt, df_trk, args.iso_dr)
    ...
    if np.any(np.isfinite(jt_iso)):
        ...
        plt.xlabel(rf"$I$ (track isolation), $\Delta R<{args.iso_dr}$")
        plt.ylabel("Entries")
        plt.title("Track isolation for jets (after overlap removal)")
        plt.tight_layout()
    ...
    if len(fpr) > 0:
        ...
        plt.title(rf"Acceptance vs Fake rate (iso cut), $\Delta R<{args.iso_dr}$")
        plt.tight_layout()
        plt.savefig(roc_png_path, dpi=200)
        print(f"Saved ROC to {roc_png_path}")

        print(f"Iso ROC summary (classify photon if iso < cut, ΔR<{args.iso_dr}):")
        print(f"  best cut (max TPR-FPR): {best_cut:.6g}")
        print(f"  best J = TPR-FPR: {best_J:.3f}")
        print(f"  at best cut: acceptance = {float(np.mean(ph_iso[np.isfinite(ph_iso)] < best_cut)):.3f}, "
              f"fake rate = {float(np.mean(jt_iso[np.isfinite(jt_iso)] < best_cut)):.3f}, "
              f"jet rejection = {1.0 - float(np.mean(jt_iso[np.isfinite(jt_iso)] < best_cut)):.3f}")
    else:
        print("[warn] could not compute ROC (need finite iso values for both photons and jets)")
